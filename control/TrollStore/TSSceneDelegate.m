	...
	for(UIOpenURLContext* context in URLContexts)
	{
		...
		if(url)
		{
			if([url isFileURL])
			{
				...
				else if([url.pathExtension.lowercaseString isEqualToString:@"deb"])
				{
					[TSPresentationDelegate startActivity:@"Installing package"];
					dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
						NSString* log = nil;
						int status = [[TSApplicationsManager sharedInstance] installDeb:url.path log:&log];
						dispatch_async(dispatch_get_main_queue(), ^{
							[TSPresentationDelegate stopActivityWithCompletion:^{
								UIAlertController* alert = [UIAlertController
									alertControllerWithTitle:status == 0 ? @"Package installed" : @"Package install failed"
									message:log ?: [NSString stringWithFormat:@"Status %d", status]
				...
				{
					// Update Commissary itself
					NSLog(@"Updating Commissary...");
					int ret = spawnRoot(rootHelperPath(), @[@"install-trollstore", url.path], nil, nil);
					doneBlock(ret == 0);
					NSLog(@"Updated Commissary!");
				}
			}
