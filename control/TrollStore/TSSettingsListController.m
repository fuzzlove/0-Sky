...
- (NSMutableArray*)specifiers
{
	if(!_specifiers)
	{
		...
		if(_newerVersion)
		{
			...
			PSSpecifier* updateTrollStoreSpecifier = [PSSpecifier preferenceSpecifierNamed:[NSString stringWithFormat:@"Update Commissary to %@", _newerVersion]
										target:self
										set:nil
										get:nil
										detail:nil
		...
		//if (@available(iOS 16, *)) { } else {
			...
			if(ldidInstalled)
			{
				[signingGroupSpecifier setProperty:@"ldid is installed and allows Commissary to install unsigned IPA files." forKey:@"footerText"];
			}
			else
			{
				[signingGroupSpecifier setProperty:@"In order for Commissary to be able to install unsigned IPAs, ldid has to be installed using this button. It can't be directly included in Commissary because of licensing issues." forKey:@"footerText"];
			}
		...
		{
			[persistenceGroupSpecifier setProperty:@"When iOS rebuilds the icon cache, all Commissary apps including Commissary itself will be reverted to \"User\" state and either disappear or no longer launch. If that happens, you can use the TrollHelper app on the home screen to refresh the app registrations, which will make them work again." forKey:@"footerText"];
		...
		else
		{
			...
			if(persistenceApp)
			{
				...
				[persistenceGroupSpecifier setProperty:[NSString stringWithFormat:@"When iOS rebuilds the icon cache, all Commissary apps including Commissary itself will be reverted to \"User\" state and either disappear or no longer launch. If that happens, you can use the persistence helper installed into %@ to refresh the app registrations, which will make them work again.", appName] forKey:@"footerText"];
			...
			{
				[persistenceGroupSpecifier setProperty:@"When iOS rebuilds the icon cache, all Commissary apps including Commissary itself will be reverted to \"User\" state and either disappear or no longer launch. The only way to have persistence in a rootless environment is to replace a system application, here you can select a system app to replace with a persistence helper that can be used to refresh the registrations of all Commissary related apps in case they disappear or no longer launch." forKey:@"footerText"];
...
#ifdef TROLLSTORE_LITE
		PSSpecifier* bluetoothGroupSpecifier = [PSSpecifier emptyGroupSpecifier];
		bluetoothGroupSpecifier.name = @"Bluetooth Fallback";
		[bluetoothGroupSpecifier setProperty:@"Keeps an encrypted, paired-Mac-only Bluetooth LE tunnel available when USB and Wi-Fi are unavailable. Initial enrollment still requires an already verified USB or Wi-Fi connection; SSH host-key and key authentication remain mandatory." forKey:@"footerText"];
		[_specifiers addObject:bluetoothGroupSpecifier];

		PSSpecifier* bluetoothToggle = [PSSpecifier preferenceSpecifierNamed:@"Enable Bluetooth Fallback"
										target:self
										set:@selector(setBluetoothFallbackEnabled:forSpecifier:)
										get:@selector(getBluetoothFallbackEnabledForSpecifier:)
										detail:nil
										cell:PSSwitchCell
										edit:nil];
		bluetoothToggle.identifier = @"bluetoothFallback";
		[_specifiers addObject:bluetoothToggle];

		PSSpecifier* bluetoothStatus = [PSSpecifier preferenceSpecifierNamed:[TSBluetoothFallback sharedFallback].statusText
										target:self set:nil get:nil detail:nil cell:PSStaticTextCell edit:nil];
		[bluetoothStatus setProperty:@NO forKey:@"enabled"];
		bluetoothStatus.identifier = @"bluetoothFallbackStatus";
		[_specifiers addObject:bluetoothStatus];

		PSSpecifier* sshAccountsGroupSpecifier = [PSSpecifier emptyGroupSpecifier];
		sshAccountsGroupSpecifier.name = @"SSH Account Passwords";
		[sshAccountsGroupSpecifier setProperty:@"Change the passwords accepted by SRDsh for new root or mobile SSH connections. Passwords are stored only as salted SHA-512 hashes; existing SSH keys continue to work." forKey:@"footerText"];
		[_specifiers addObject:sshAccountsGroupSpecifier];

		PSSpecifier* rootPasswordSpecifier = [PSSpecifier preferenceSpecifierNamed:@"Change Root Password"
										target:self
										set:nil
										get:nil
										detail:nil
										cell:PSButtonCell
										edit:nil];
		rootPasswordSpecifier.identifier = @"changeRootPassword";
		[rootPasswordSpecifier setProperty:@YES forKey:@"enabled"];
		rootPasswordSpecifier.buttonAction = @selector(changeRootPasswordPressed);
		[_specifiers addObject:rootPasswordSpecifier];

		PSSpecifier* mobilePasswordSpecifier = [PSSpecifier preferenceSpecifierNamed:@"Change Mobile Password"
										target:self
										set:nil
										get:nil
										detail:nil
										cell:PSButtonCell
										edit:nil];
		mobilePasswordSpecifier.identifier = @"changeMobilePassword";
		[mobilePasswordSpecifier setProperty:@YES forKey:@"enabled"];
		mobilePasswordSpecifier.buttonAction = @selector(changeMobilePasswordPressed);
		[_specifiers addObject:mobilePasswordSpecifier];
#endif
...
#ifndef TROLLSTORE_LITE
		// Uninstall Commissary
		PSSpecifier* uninstallTrollStoreSpecifier = [PSSpecifier preferenceSpecifierNamed:@"Uninstall Commissary"
										target:self
										set:nil
										get:nil
										detail:nil
...
#ifdef TROLLSTORE_LITE
- (NSNumber*)getBluetoothFallbackEnabledForSpecifier:(PSSpecifier*)specifier
{
	(void)specifier;
	id value = [[NSUserDefaults standardUserDefaults] objectForKey:@"0SkyBluetoothFallbackEnabled"];
	return value ? @([value boolValue]) : @YES;
}

- (void)setBluetoothFallbackEnabled:(NSNumber*)value forSpecifier:(PSSpecifier*)specifier
{
	(void)specifier;
	[[NSUserDefaults standardUserDefaults] setBool:value.boolValue forKey:@"0SkyBluetoothFallbackEnabled"];
	if(value.boolValue) [[TSBluetoothFallback sharedFallback] start];
	else [[TSBluetoothFallback sharedFallback] stop];
	dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.4 * NSEC_PER_SEC)),
		dispatch_get_main_queue(), ^{ [self reloadSpecifiers]; });
}

- (void)showPasswordResultWithTitle:(NSString*)title message:(NSString*)message
{
...
- (void)promptForPasswordForAccount:(NSString*)account displayName:(NSString*)displayName
{
	UIAlertController* alert = [UIAlertController
		alertControllerWithTitle:[NSString stringWithFormat:@"Change %@ Password", displayName]
		message:@"Enter the new password twice. It must be 8-128 UTF-8 bytes."
	...
	[alert addAction:[UIAlertAction actionWithTitle:@"Change Password"
		style:UIAlertActionStyleDefault handler:^(UIAlertAction* action) {
		(void)action;
		NSString* password = [alert.textFields[0].text copy] ?: @"";
		NSString* confirmation = [alert.textFields[1].text copy] ?: @"";
		alert.textFields[0].text = @"";
		alert.textFields[1].text = @"";
		NSUInteger byteCount = [password lengthOfBytesUsingEncoding:NSUTF8StringEncoding];
		if(![password isEqualToString:confirmation])
		{
			[self showPasswordResultWithTitle:@"Passwords Do Not Match"
				message:@"The password was not changed."];
			return;
		}
		if(byteCount < 8 || byteCount > 128 ||
		   [password rangeOfCharacterFromSet:NSCharacterSet.newlineCharacterSet].location != NSNotFound)
		{
			[self showPasswordResultWithTitle:@"Invalid Password"
				message:@"Use 8-128 UTF-8 bytes without line breaks."];
			return;
		}
		self.view.userInteractionEnabled = NO;
		dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
			NSString* error = nil;
			int status = setDeviceAccountPassword(account, password, &error);
			dispatch_async(dispatch_get_main_queue(), ^{
				self.view.userInteractionEnabled = YES;
				if(status == 0)
				{
					[self showPasswordResultWithTitle:@"Password Changed"
						message:[NSString stringWithFormat:@"New SSH connections for %@ will use the new password.", account]];
				}
				else
				{
					[self showPasswordResultWithTitle:@"Password Change Failed"
						message:error.length ? error : [NSString stringWithFormat:@"0-Sky returned status %d.", status]];
				}
			});
		});
	}]];
...
- (void)changeRootPasswordPressed
{
	[self promptForPasswordForAccount:@"root" displayName:@"Root"];
}

- (void)changeMobilePasswordPressed
{
	[self promptForPasswordForAccount:@"mobile" displayName:@"Mobile"];
}
#endif
...
- (void)installPersistenceHelperPressed
{
	...
	UIAlertController* selectAppAlert = [UIAlertController alertControllerWithTitle:@"Select App" message:@"Select a system app to install the Commissary Persistence Helper into. The normal function of the app will not be available, so it is recommended to pick something useless like the Tips app." preferredStyle:UIAlertControllerStyleActionSheet];
