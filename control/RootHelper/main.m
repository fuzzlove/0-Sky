...
int installApp(NSString* appPackagePath, BOOL sign, BOOL force, BOOL isTSUpdate, BOOL useInstalldMethod, BOOL skipUICache)
{
	...
	if(appContainer)
	{
		...
		{
			NSLog(@"[installApp] already installed and not a Commissary app... bailing out");
			return 171;
		}
		...
		if(!isTSUpdate)
		{
			BKSTerminateApplicationForReasonAndReportWithDescription(appId, 5, false, @"Commissary - App updated");
		}
	...
	if(![[NSFileManager defaultManager] fileExistsAtPath:trollStoreMarkURL.path])
	{
		...
		if(!marked)
		{
			NSLog(@"[installApp] failed to mark %@ as Commissary app by creating %@, error: %@", appId, trollStoreMarkURL.path, creationError);
			return 177;
		}
	}
...
int installTrollStore(NSString* pathToTar)
{
	...
	NSLog(@"[installCommissary] installApp => %d", ret);
	[[NSFileManager defaultManager] removeItemAtPath:tmpPackagePath error:nil];
	return ret;
}
...
void installPersistenceHelper(NSString* systemAppId, NSString *persistenceHelperBinary, NSString *rootHelperBinary)
{
	...
	BKSTerminateApplicationForReasonAndReportWithDescription(systemAppId, 5, false, @"Commissary - Reload persistence helper");
}
...
void uninstallPersistenceHelper(void)
{
	...
	if(systemAppProxy)
	{
		...
		BKSTerminateApplicationForReasonAndReportWithDescription(systemAppProxy.bundleIdentifier, 5, false, @"Commissary - Reload persistence helper");
	}
...
int MAIN_NAME(int argc, char *argv[], char *envp[])
{
	@autoreleasepool {
		...
		if(getuid() != 0)
		{
			NSLog(@"ERROR: Commissary helper has to be run as root.");
			return -1;
		}
		...
		NSLog(@"Commissary helper invoked with arguments: %@", args);
		...
		NSLog(@"Commissary helper returning %d", ret);
		return ret;
	}
}
