...
// Newer SDK headers mark the private iOS client declaration unavailable at
// compile time. Keep the upstream runtime symbol but use a project-local name
// so a current Xcode toolchain can build the authorized SRD helper.
extern xpc_connection_t zs_xpc_connection_create_mach_service(
...
xpc_connection_t startConnection(void) {
	xpc_connection_t connection = zs_xpc_connection_create_mach_service("com.apple.amfi.xpc", NULL, 0);
    if (!connection) {
