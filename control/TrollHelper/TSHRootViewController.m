...
- (NSMutableArray*)specifiers
{
	if(!_specifiers)
	{
		...
		PSSpecifier* infoSpecifier = [PSSpecifier preferenceSpecifierNamed:@"Commissary"
											target:self
											set:nil
		...
		{
			PSSpecifier* installTrollStoreSpecifier = [PSSpecifier preferenceSpecifierNamed:@"Install Commissary"
												target:self
												set:nil
												get:nil
												detail:nil
		...
		if((isRegistered || !persistenceHelperProxy) && ![[NSFileManager defaultManager] fileExistsAtPath:@"/Applications/TrollStorePersistenceHelper.app"])
		{
			...
			if(isRegistered)
			{
				bottomText = @"This app is registered as the Commissary persistence helper and can be used to fix Commissary app registrations in case they revert back to \"User\" state and the apps say they're unavailable.";
			...
			{
				bottomText = @"If you want to use this app as the Commissary persistence helper, you can register it here.";
	...
	[(UINavigationItem *)self.navigationItem setTitle:@"Commissary Helper"];
	return _specifiers;
}
